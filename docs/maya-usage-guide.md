...
Step-by-step instructions for using the `maya_grass` plugin to generate animated grass in Autodesk Maya.
...
```bash
# copy the modules to Maya's scripts directory
cp -r src/maya_grass ~/Documents/maya/2024/scripts/
cp -r src/generative_art ~/Documents/maya/2024/scripts/
```
...
```python
import sys
sys.path.insert(0, '/path/to/generative-art/src')
```
...
```python
import sys
sys.path.insert(0, '/path/to/generative-art/src')  # adjust path
...
| Issue | Solution |
|-------|----------|
| `ModuleNotFoundError: No module named 'maya_grass'` | Check `sys.path` includes the `src` directory |
...
```python
from maya_grass import WindField
...
```python
from maya_grass import TerrainAnalyzer

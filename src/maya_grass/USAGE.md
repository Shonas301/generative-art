...
`maya_grass` generates animated grass across a terrain mesh using MASH instancing. Wind animation is driven by opensimplex noise, and grass can optionally cluster around obstacles (simulating foot traffic avoidance).
...
```
generative-art/src/maya_grass/       → scripts/maya_grass/
generative-art/src/generative_art/   → scripts/generative_art/
```
